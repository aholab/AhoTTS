...
======================== Contenido ========================
<DOC>
Metodos de {{LangES_Prosod}} que implementan diferentes
...
PRIVATE VOID utt_dur_unnorm(UttPh & ut)
{
	...
	for (p = ut.phoneFirst(); p != 0; p = ut.phoneNext(p)) {
		...
		default:
			htts_warn("es_dur1: Phone not known (%s).",phone_tosampa(ut.cell(p).getPhone()));
			d=90; break;
		}
...
VOID LangES_Prosod::utt_dur1(UttPh & ut)
{
	UttI p;
	LONG N;
	DOUBLE kacc, kstep, k;
