...
BOOL LangES_HDicDB::flagParse( const CHAR *s, HDicRef &r )
{
	...
	else if (!strcmp(s,"ir1c")) r.setquery(HDIC_QUERY_ES_VERB, 5); // Infinitivos Regulares 1ª Conjugación
	else if (!strcmp(s,"ir2c")) r.setquery(HDIC_QUERY_ES_VERB, 6); // Infinitivos Regulares 2ª Conjugación
	else if (!strcmp(s,"ir3c")) r.setquery(HDIC_QUERY_ES_VERB, 7); // Infinitivos Regulares 3ª Conjugación
	else if (!strcmp(s,"iicc")) r.setquery(HDIC_QUERY_ES_VERB, 8); // Infinitivos Irregulares Cualquier Conjugación (verbos inventados para el "conjugador")
	else if (!strcmp(s,"otro")) r.setquery(HDIC_QUERY_ES_OTRO, 1); // Otros (SUStantivos, adjetivos... para desambigüar raices de verbos)
