...
typedef struct {
	...
	DOUBLE kdf, kdi;  // correccion desviacion de pico: 0=normal, hasta -1/1=inicial/final
...
VOID LangES_Prosod::utt_pth1( UttPh &ut )
{
	Pth1CfgSent prms;
	UttI last, first;
	UttCfg * cfg=0;
    ...
    case USENTENCE_COLON :  cfg =  &utt_decl;break; // tipo _
    case USENTENCE_SCOLON : cfg =  &utt_decl;break; // tipo _
    case USENTENCE_SUSPEN : cfg =  &utt_decl;break; // tipo _
    case USENTENCE_PAR :  cfg =  &utt_decl;break; // tipo _
    case USENTENCE_PAROPEN : cfg = &utt_pause; break;
		default:htts_error("LangES_Prosod: Unknown sentence type"); break;
	}
	...
	for (p=first; p!=0; p=ut.sentenceNext(p)) {
	    ...
	    case USENTENCE_COLON :  scfg = &sent_decl;break; // tipo _
  	  case USENTENCE_SCOLON : scfg = &sent_decl;break; // tipo _
    	case USENTENCE_SUSPEN : scfg = &sent_decl;break; // tipo _
	    case USENTENCE_PAR :  scfg = &sent_decl;break; // tipo _
...
#define DESV(s) linterpol(s,0,nsil-1,prm.d0,prm.d2)
	...
	for (p = first; p != 0; p = ut.emphasisNext(p,URANGE_SENTENCE)) {
		...
		ut.cell(LangES_PhUti::syllable_xvowel(ut,p)).setPth(MEAN(s)+DESV(s));
	}

	/* ponemos el valor inicial (cabeza) */
	UttI vbegin = LangES_PhUti::syllable_xvowel(ut,begin);  // vocal en la silaba
	...
	UttI vend = LangES_PhUti::syllable_xvowel(ut,end);  // vocal en la silaba
...
#endif
					if (pv1) ut.cell(LangES_PhUti::syllable_xvowel(ut,pv1)).setPth(p1);
					if (pv2) ut.cell(LangES_PhUti::syllable_xvowel(ut,pv2)).setPth(p2);
				}
				else {
					LONG ss=(s1+s2+1)/2;
...
#endif
					DOUBLE pp1=linterpol(ss,s,s1,ut.cell(LangES_PhUti::syllable_xvowel(ut,p)).getPth(),p1);
					DOUBLE pp2=linterpol(ss,s2,snext,p2,ut.cell(LangES_PhUti::syllable_xvowel(ut,next)).getPth());
					if (pp1<pp2) pp1=pp2;
					if (pss) ut.cell(LangES_PhUti::syllable_xvowel(ut,pss)).setPth(pp1);
				}
			}
...
#endif
				if (pv) ut.cell(LangES_PhUti::syllable_xvowel(ut,pv)).setPth(MEAN(sv)-DESV(sv));
			}
