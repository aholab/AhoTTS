...
0.0.2    15/11/02  inigos    Recopia y adaptación para castellano
...
CtI LangES_TextToList::dateFromSpanish(CtI p)
{
	CtI pp, qq;
	pCHAR day;
	pCHAR year;
...
CtI LangES_TextToList::dateFromRoman(CtI p)
{
	CtI q;
	UINT num;
	CHAR temp[80];
...
CtI LangES_TextToList::dateFromRomanSpanish(CtI p)
{
	p = dateFromRoman(p);
	p = dateFromSpanish(p);
...
DateType LangES_TextToList::isDate(CtI p)
{
