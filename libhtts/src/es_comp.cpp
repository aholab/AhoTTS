...
-------  --------  --------  -----------------------
0.0.7    21/10/11  Inaki     la funcion normal de eu_normal.cpp se encargara de hacerlas pronunciables (se pierde la marca UTYPENOR_COMP, aunque no la usamos para nada)
...
0.0.6    22/05/05  Larraitz  Pattern arazo bat konpondu
0.0.5    04/05/04  inigos    Recopia para castellano
...
CtI LangES_TextToList::expComp( CtI p )
{
	CtI q;
	INT nint = 0 ,i;
	INT patlen;
	...
	//strcpy(pattern, ct(p).str); Larraitz
...
/*		case 2:
			...
			q=ct.next(p);
			number = strdup(ct(q).str);

			// Borramos el compuesto
			q=ct.delGrp(p);

			// Número
			q=ct.insbefore(q, number, FALSE);
			q = expnum(q);
			q=ct.firstInGrp(q);
			q = ct.del(q);

			// Palabra
			q=ct.insbefore(q, word, FALSE);
			q = spellCell(q, q);
			q=ct.firstInGrp(q);
			q = ct.del(q);

			// Patrón, estado y tipo de normalización de la palabra (al número se lo ponen)
			ct.setPatternForce(q, "l");
			ct.setStatus(q, stat);
			ct.setTnor(q, UTYPENOR_COMP);
		...
		default:
#ifdef HTTS_DEBUG
			htts_error("LangES_TextToList::expComp() No valid pattern length detected.");
#endif
			break;
	}
	...
	p=ct.lastInGrp(q);
	for (i = 0; i < nint; i++)	{
		if (isPronun(q)) p=ct.insafter(p, ct.getStr(q), FALSE, emotion, emo_intensity);//inaxio-> emotion eta emo_intensity gehitu
