...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\*/
/******************************************************************************/
#ifndef __ES_TEXT2LIST_HPP__
#define __ES_TEXT2LIST_HPP__
...
-------  --------  --------  -----------------------
0.1.5	 02/01/07  inaxio    añadido expSuma desde es_numhilvl.
0.1.4    29/12/06  inaxio    expNegative gehitua es_numhilvl-etik.
0.1.3	 20/11/06  inaxio    zuzenketa: CtI expUnit( CtI p , BOOL junto )->BOOL junto gehitu da
0.1.2	 25/10/06  inaxio    zuzenketa: CtI expPercent( CtI p , BOOL junto )->BOOL junto gehitu da
0.1.1	 05/10/06  inaxio    Aldaketa zenbaki ordinal pluralentzat.
0.1.0	 11/11/02  inigos    Adaptación al castellano (copia de eu_t2l.hpp)
...
class LangES_TextToList : public TextToList {
...
public:
	...
	CtI upTo99(UINT num, CtI p, BOOL primero);
	...
	CtI expCard(CtI p);
	CtI expOrd(CtI p, BOOL female, BOOL plural);
