...
-------  --------  --------  -----------------------
0.0.2	 02/10/07  Inaki      Añadir u con dieresis como tipo de letra
0.0.1	 29/12/06  inaxio     puntChop-en aldaketak zenbaki negatiboak identifikatzeko.
0.0.1	 28/07/05  Nora	      Correccion en la funcion puntChop.
0.0.0    11/10/04  inigos     Codificacion inicial.

======================== Contenido ========================

Funciones de wordchop.cpp separadas por dependencia idiomatica.

Correccion 28/07/05:
	En diferentes pruebas daba un error de violacion de segmento. 
	Causa: Dentro de la funcion "puntChop" en un momento se accede a un puntero NULL, 
	porque no se daba una comprobacion previa. Los cambios estan marcados con el 
	comentario // NORA.
...
INT es_chtype[256] = {
	...
	/* [162] ¢ */ CHTYPE_NULL, 
	/* [163] £ */ CHTYPE_SYMBL,
	/* [164] ¤ */ CHTYPE_SYMBL, //INAKI --> simbolo euro
	/* [165] ¥ */ CHTYPE_SYMBL,
	/* [166] ¦ */ CHTYPE_NULL,
	/* [167] § */ CHTYPE_NULL,
	/* [168] ¨ */ CHTYPE_NULL,
...
INT LangES_TextToList::getchtype(CHAR c)
{
	void *j = &c;
	return es_chtype[ *((pUCHAR)j)];
}
...
CtI LangES_TextToList::puntChop(HDicDB *dic)
{
	CtI p, firstNonP, lastToNoP,p2;
	...
	for(p=firstNonP; p!=lastToNoP; p=ct.nextGrp(p))	
	{
		...
		if (hasRightPuntt(ct(p).pattern))
		{
			...
			if (!len)
			{
				...
				if (abbacr != HDIC_ANSWER_ES_NOR_ABBNF && abbacr != HDIC_ANSWER_ES_NOR_ABB)
				{
					patCutRight(p);
				}
				else // Hay que mirar si es lo último
				{
					if (abbacr == HDIC_ANSWER_ES_NOR_ABBNF)
					{
						ct.setTnor(p, UTYPENOR_ACROABBUNI);
					}
					if(abbacr == HDIC_ANSWER_ES_NOR_ABB)
					{
					...
					else
					{
						...
						if(p2) //NORA
						{ //NORA
							// Si el siguiente está en mayúsculas cortamos
							if ((ct(q).pattern)[0] == 'l')
							{
								if (ct.getStr(q)[0] != ToLower(ct.getStr(q)[0]) )
								{
									// Siempre que el tipo de abreviatura nos deje
									if(abbacr == HDIC_ANSWER_ES_NOR_ABB)
									{
										patCutRight(p);	
									}
